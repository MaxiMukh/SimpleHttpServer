...
public class ImdbApiHandler implements HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if (exchange.getRequestURI().getQuery() == null || exchange.getRequestURI().getQuery().isEmpty()) {
			// Get search string if request looks like http://127.0.0.1/imdb/The%20Game%20of%20Thrones
			query = exchange.getRequestURI().getPath().split("/")[2];
		} else {
			// Get search string if request looks like http://127.0.0.1/imdb/?q=The%20Game%20of%20Thrones
			query = exchange.getRequestURI().getQuery().split("=")[1];
		}
		exchange.getRequestBody().close();
		try {
			// Encoding concatenated address to correct URL
			String address = ConfigHttpServer.IMDB_HOST.concat(URLEncoder.encode(query, "UTF-8"));
